...
public class HellscraftModVillagerProfessions {
	private static final Map<String, ProfessionPoiType> POI_TYPES = new HashMap<>();
	public static final DeferredRegister<VillagerProfession> PROFESSIONS = DeferredRegister.create(ForgeRegistries.VILLAGER_PROFESSIONS, HellscraftMod.MODID);
	public static final RegistryObject<VillagerProfession> NECROMANCER = registerProfession("necromancer", () -> HellscraftModBlocks.BLOCK_OF_VILLAGER.get(),
			() -> ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.sculk_vein.place")));
	...
	private static RegistryObject<VillagerProfession> registerProfession(String name, Supplier<Block> block, Supplier<SoundEvent> soundEvent) {
		...
		return PROFESSIONS.register(name, () -> {
			...
			return new VillagerProfession(HellscraftMod.MODID + ":" + name, poiPredicate, poiPredicate, ImmutableSet.of(), ImmutableSet.of(), soundEvent.get());
		});
	}
		...
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			for (Map.Entry<String, ProfessionPoiType> entry : POI_TYPES.entrySet()) {
				...
				if (existingCheck.isPresent()) {
					HellscraftMod.LOGGER.error("Skipping villager profession " + name + " that uses POI block " + block + " that is already in use by " + existingCheck);
					continue;
				}
